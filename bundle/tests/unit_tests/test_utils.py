...
def test_cleanse_data(spark):
    ...
    # Checks missing categorical values are filled with "unknown"
    assert cleaned_df.filter(cleaned_df.id == 2).select("category").first()[0] == "unknown"

    # Checks rows with null integer fields are filtered out
    assert cleaned_df.filter(cleaned_df.integer_field.isNull()).count() == 0

def test_calculate_turbine_summary(spark):
    data = [
    ...
    # Checks average power calculation
...
def test_identify_anomalies(spark):
    ...
    # Checks if anomalies are identified correctly
